...
class ModuleResolver:
    ...
    def resolve(
        ...
        Args:
            ...
            unpinned: If True, allow unpinned branch refs on remote repos (resolved
        ...
        if _is_local:
            ...
            # Remote URL: commit or pinned tag required unless --unpinned is set.
            if not repo.commit or repo.commit.strip() == "":
                raise RuntimeError(
                    ...
                    "or use --unpinned to resolve branch references at run time."
                )
            if not is_pinned and not unpinned:
                raise RuntimeError(
                    f"Module '{module_id}' uses unpinned reference '{repo.commit}'. "
                    "For reproducibility, use a commit hash (e.g., 'abc1234') or "
                    "tag (e.g., 'v1.0.0'). Use --unpinned to allow branch references."
                )
