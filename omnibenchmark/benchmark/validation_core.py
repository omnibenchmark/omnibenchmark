...
def validate_license_consistency(
    ...
    if citation_license and not license_content:
        # This is typically a warning
        warning_issue = ValidationIssue(
            issue_type="license_in_citation_but_not_file",
            severity=ValidationSeverity.WARNING,
            path="LICENSE",
            module_id=ctx.module_id,
            message="License specified in CITATION.cff but no LICENSE file found",
        )
        result.add_issue(warning_issue)
        return result
...
def validate_file_structure(
    files_present: Dict[str, bool], ctx: ValidationContext
) -> ValidationResult:
    ...
    # Check for LICENSE file (various names)
    license_files = [
        "LICENSE",
        "LICENSE.txt",
        "LICENSE.md",
