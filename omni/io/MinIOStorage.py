...
class MinIOStorage(RemoteStorage):
    ...
    def _update_overview(self, cleanup=True):
        ...
        for v in versions_not_in_overview:
            result = self.client.put_object(f"{self.benchmark}.overview", v, io.BytesIO(b""), 0)
        ...
        if cleanup:
            ...
            deletes = self.client.remove_objects(f"{self.benchmark}.overview",versions_in_overview_but_unavailable)
    ...
    def _create_new_version(self):
        ...
        if not f"{self.major_version_new}.{self.minor_version_new}" in self.versions:
            # create new version
            self.client.make_bucket(bucket_name=f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}")
            set_bucket_public_readonly(self.client, f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}")
            if not self.client.bucket_exists(f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}"):
        ...
        if not 'secret_key' in self.auth_options.keys() or readonly:
            ...
            for obj in names:
                ...
                if self.auth_options['secure']:
                    ...
                    url = url._replace(scheme = "http")
                mtime, accesstime = get_meta_mtime(url.geturl(), containername, obj.find('Key').text)
            ...
            for element in self.client.list_objects(f"{self.benchmark}.{self.major_version}.{self.minor_version}", recursive=True):
                print(element)
                if type(element.last_modified) is str:
                    files[element.object_name] = {'size': element.size, 'last_modified': element.last_modified, 'hash': element.etag.replace('"',''), 'symlink_path': ''}
                elif type(element.last_modified) is datetime.datetime:
                    files[element.object_name] = {'size': element.size, 'last_modified': element.last_modified.strftime('%Y-%m-%dT%H:%M:%S.%f'), 'hash': element.etag.replace('"',''), 'symlink_path': ''}
                else: 
    ...
    def copy_objects(self, type = "copy"):
            ...
            if type == "copy":
                ...
                for filename in filenames:
                    out = self.client.copy_object(f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}", filename, minio.commonconfig.CopySource(bucket_name=f"{self.benchmark}.{self.major_version}.{self.minor_version}", object_name=filename))
