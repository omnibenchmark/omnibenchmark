...
class SwiftStorage(RemoteStorage):
    ...
    def connect(self):
        ...
        """
        if (
            "preauthurl" in self.auth_options.keys()
            and "preauthtoken" in self.auth_options.keys()
        ):
    ...
    def _create_benchmark(self, benchmark, update=True):
        ...
        with self.connect() as swift:
            post = swift.post(
                container=f"{benchmark}.test.1", options={"read_acl": ".r:*,.rlistings"}
            )
            if not post["success"]:
                raise SwiftError(
                    f"Benchmark creation of {benchmark}.test.1 failed: {post['error']}"
                )
            post = swift.post(
                container=f"{benchmark}.overview",
                options={"read_acl": ".r:*,.rlistings"},
            )
            if not post["success"]:
                raise SwiftError(
                    f"Benchmark creation of {benchmark}.overview failed: {post['error']}"
                )
            post = swift.post(
                container=f"{benchmark}.0.1", options={"read_acl": ".r:*,.rlistings"}
            )
            if not post["success"]:
                raise SwiftError(
    ...
    def _update_overview(self, cleanup=True):
        ...
        with self.connect() as swift:
            for page in swift.list(
                container=f"{self.benchmark}.overview", options={"long": True}
            ):
                for element in page["listing"]:
        ...
        with self.connect() as swift:
            for v in versions_not_in_overview:
                ...
                if not post["success"]:
                    raise SwiftError(
                        f"Object creation of {self.benchmark}.overview/{v} failed: {post['error']}"
                    )
        ...
        if cleanup:
            ...
            versions_in_overview_but_unavailable = [
                v for v in versions_in_overview if v not in self.versions
            ]
            with self.connect() as swift:
                deletes = swift.delete(
                    container=f"{self.benchmark}.overview",
                    objects=versions_in_overview_but_unavailable,
                )
                for delete in deletes:
                    if not delete["success"]:
                        raise SwiftError(
                            f"Object deletion of {self.benchmark}.overview/{v} failed: {delete['error']}"
                        )
    ...
    def _create_new_version(self):
        ...
        if not f"{self.major_version_new}.{self.minor_version_new}" in self.versions:
            ...
            with self.connect() as swift:
                post = swift.post(
                    container=f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}",
                    options={"read_acl": ".r:*,.rlistings"},
                )
                if not post["success"]:
                    raise SwiftError(
                        f"Version creation of {self.benchmark}.{self.major_version_new}.{self.minor_version_new} failed: {post['error']}"
                    )
                # except SwiftError as e:
                #     logger.error(e.value)
    ...
    def _get_objects(self, readonly=False):
        ...
        if not "preauthtoken" in self.auth_options.keys() or readonly:
            ...
            with self.connect() as swift:
                ...
                # get all objects
                for page in swift.list(
                    container=f"{self.benchmark}.{self.major_version}.{self.minor_version}",
                    options={"long": True},
                ):
                    for element in page["listing"]:
                        files[element["name"]] = {
                        ...
                        if "symlink_path" in element.keys():
                            files[element["name"]]["symlink_path"] = element[
                                "symlink_path"
                            ]
                        else:
    ...
    def copy_objects(self, type="copy"):
        ...
        if type == "copy":
            with self.connect() as swift:
                ...
                for out in swift.copy(
                    f"{self.benchmark}.{self.major_version}.{self.minor_version}",
                    filenames,
                    {
                        "destination": f"/{self.benchmark}.{self.major_version_new}.{self.minor_version_new}"
                    },
                ):
