...
def list_files(benchmark: str, type: str = None, stage: str = None, module: str = None, file_id: str = None, version: str = None, verbose: bool = False):
    ...
    if __name__ == "__main__":
        minio_auth_options_public = {'endpoint': 'https://omnibenchmark.mls.uzh.ch:9000', 'secure': False }
        bench_yaml = {'auth_options': minio_auth_options_public, 'storage_type': 'minio'}
    ...
    if version is None:
        ...
        # set version
        ss.set_current_version(major_version=int(version.split(".")[0]), minor_version=int(version.split(".")[1]))
    ...
    urls = {} 
    for name in names:
        url = urlparse(f"{ss.auth_options['endpoint']}/{ss.benchmark}.{ss.major_version}.{ss.minor_version}/{name}")
        if 'secure' in ss.auth_options.keys() and not ss.auth_options['secure']:
            url = url._replace(scheme = "http")
        else:
            url = url._replace(scheme = "https")
        urls[name] = {'url': url.geturl(), 'size': int(ss.files[name]['size']), 'md5': ss.files[name]['hash']}
    return urls
...
def download_files(benchmark: str, type: str = None, stage: str = None, module: str = None, file_id: str = None, version: str = None, verbose: bool = False):
...
def checksum_files(benchmark: str, type: str, stage: str, module: str, file_id: str, version: str, verbose: bool = False):
