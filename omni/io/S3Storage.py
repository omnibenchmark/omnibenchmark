...
class S3Storage(RemoteStorage):
    ...
    def connect(self):
        ...
        """
        if  'endpoint' in self.auth_options.keys() and 'access_key' in self.auth_options.keys() and 'secret_key' in self.auth_options.keys():
    ...
    def _update_overview(self, cleanup=True):
        ...
        for element in self.client.Bucket(f"{self.benchmark}.overview").objects.iterator():
            versions_in_overview.append(element.key)
        # available benchmark versions
        self._get_versions()
        # missing versions
        versions_not_in_overview = [v for v in self.versions + self.other_versions if v not in versions_in_overview ]
        # create missing versions
        for v in versions_not_in_overview:
            result = self.client.Bucket(f"{self.benchmark}.overview").put_object(Key=v, Body=b"")
        ...
        if cleanup:
            ...
            versions_in_overview_but_unavailable = [v for v in versions_in_overview if v not in self.versions + self.other_versions ]
            if len(versions_in_overview_but_unavailable) > 0:
                deletes = self.client.Bucket(f"{self.benchmark}.overview").delete_objects(Delete={'Objects': [{'Key': v} for v in versions_in_overview_but_unavailable]})
    ...
    def _create_new_version(self):
        ...
        if not f"{self.major_version_new}.{self.minor_version_new}" in self.versions:
            # create new version
            self.client.create_bucket(Bucket=f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}")
            self._set_bucket_public_readonly(f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}")
            if self.client.Bucket(f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}").creation_date is None:
                raise Exception(f"Benchmark creation of {self.benchmark}.{self.major_version_new}.{self.minor_version_new} failed")
            # update
        ...
        if not 'secret_key' in self.auth_options.keys() or readonly:
            ...
            for obj in names:
                ...
                if self.auth_options['secure']:
                    ...
                    url = url._replace(scheme = "http")
                mtime, accesstime = self._get_meta_mtime(url.geturl(), containername, obj.find('Key').text)
            ...
            for element in self.client.Bucket(f"{self.benchmark}.{self.major_version}.{self.minor_version}").objects.iterator():
                print(element)
                if type(element.last_modified) is str:
                    files[element.key] = {'size': element.size, 'last_modified': element.last_modified, 'hash': element.e_tag.replace('"',''), 'symlink_path': ''}
                elif type(element.last_modified) is datetime.datetime:
                    files[element.key] = {'size': element.size, 'last_modified': element.last_modified.strftime('%Y-%m-%dT%H:%M:%S.%f'), 'hash': element.e_tag.replace('"',''), 'symlink_path': ''}
                else: 
    ...
    def copy_objects(self, type = "copy"):
            ...
            if type == "copy":
                ...
                for filename in filenames:
                    out = self.client.Object(f"{self.benchmark}.{self.major_version_new}.{self.minor_version_new}", filename).copy_from(CopySource={'Bucket': f"{self.benchmark}.{self.major_version}.{self.minor_version}", 'Key': filename})
