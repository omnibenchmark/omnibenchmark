...
def cleanup_buckets():
    ...
    tmp_auth_options['endpoint'] = tmp_auth_options['endpoint'].replace("http://", "").replace("https://", "")
...
def test__get_objects():
    ss = MinIOStorage(auth_options=auth_options, benchmark="test")
    result = ss.client.put_object(f"{ss.benchmark}.0.1", "file1.txt", io.BytesIO(b""), 0)
    result = ss.client.put_object(f"{ss.benchmark}.0.1", "file2.txt", io.BytesIO(b""), 0)
    ss.set_current_version()
    ss._get_objects()
    print(ss.files)
...
def test__get_objects_public():
    ...
    print(ss.files)
    assert ss.files.keys() == {'file1.txt', 'file2.txt'}
    assert ss.files['file1.txt'].keys() == {'hash', 'last_modified', 'size', 'x-object-meta-mtime', 'symlink_path', 'accesstime'}
