...
class MockbucketsCollectionManager():
    def __init__(self):
        self.buckets = [MockS3Bucket('test.0.1'), MockS3Bucket('test.0.2'), MockS3Bucket('test.1.0'), MockS3Bucket('test2.0.1')]
    def all(self):
        return self.buckets
    def iterator(self):
        return self.buckets
...
class MockS3Bucket():
    ...
    def put_object(self, Key, Body):
        return {'success': True, 'bucket': self.name, 'object': Key, 'action': 'put_object'}
    def delete_objects(self, Delete):
        return {'success': True, 'bucket': self.name, 'objects': Delete, 'action': 'delete_objects'}
    def Policy(self):
...
class MockS3Policy():
    ...
    def delete(self):
        return {'success': True, 'bucket': self.bucket, 'action': 'delete_bucket_policy'}
...
class MockResponse():
    def __init__(self, url):
        ...
        self.headers = {'Accept-Ranges': 'bytes', 'Content-Length': '2', 'Content-Type': 'application/octet-stream', 'ETag': '"49f68a5c8493ec2c0bf489821c21fc3b"', 'Last-Modified': 'Fri, 03 May 2024 10:44:26 GMT', 'Server': 'MinIO', 'Strict-Transport-Security': 'max-age=31536000; includeSubDomains', 'Vary': 'Origin, Accept-Encoding', 'X-Amz-Id-2': 'dd9025bab4ad464b049177c95eb6ebf374d3b3fd1af9251148b658df7ac2e3e8', 'X-Amz-Request-Id': '17D045E2CA527BB1', 'X-Content-Type-Options': 'nosniff', 'X-Xss-Protection': '1; mode=block', 'x-amz-tagging-count': '2', 'Date': 'Fri, 17 May 2024 12:18:18 GMT'}
...
def test__test_connect(monkeypatch):
    ...
    ss = S3Storage(auth_options={'endpoint': 'http://omnibenchmark.mls.uzh.ch:9000', 'access_key': 'asdfasdf', 'secret_key': 'asdfasdfasdf', 'secure': False}, benchmark="test")
...
def test_init_public(monkeypatch):
    ...
    ss = S3Storage(auth_options={'endpoint': 'http://omnibenchmark.mls.uzh.ch:9000', 'secure': False}, benchmark="test")
...
def test__get_objects(monkeypatch):
    ...
    assert ss.files['file1.txt'].keys() == {'hash', 'last_modified', 'size', 'symlink_path'}
    assert ss.files['file2.txt'].keys() == {'hash', 'last_modified', 'size',  'symlink_path'}
...
def test_find_objects_to_copy(monkeypatch):
    ...
    assert ss.files.keys() == {'file1.txt', 'file2.txt'}
    assert ss.files['file1.txt'].keys() == {'hash', 'last_modified', 'size', 'symlink_path', 'copy'}
    assert ss.files['file2.txt'].keys() == {'hash', 'last_modified', 'size', 'symlink_path', 'copy'}
...
def test_copy_objects(monkeypatch):
    ...
    ss.copy_objects()
    assert ss.files.keys() == {'file1.txt', 'file2.txt'}
    assert ss.files['file1.txt'].keys() == {'hash', 'last_modified', 'size', 'symlink_path', 'copy', 'copied'}
    assert ss.files['file2.txt'].keys() == {'hash', 'last_modified', 'size', 'symlink_path', 'copy', 'copied'}
