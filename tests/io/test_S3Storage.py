...
def test__get_objects():
    ss = S3Storage(auth_options=auth_options, benchmark="test")
    result = ss.client.Bucket(f"{ss.benchmark}.0.1").put_object(Key="file1.txt", Body=b"")
    result = ss.client.Bucket(f"{ss.benchmark}.0.1").put_object(Key="file2.txt", Body=b"")
    ss.set_current_version()
    ss._get_objects()
    print(ss.files)
