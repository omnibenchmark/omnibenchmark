...
class TestSubstituteProvides:
    ...
    def test_unknown_provides_raises(self):
        ctx = TemplateContext(provides={"dataset": "iris"})
        with pytest.raises(ValueError, match="unknown"):
            ctx.substitute("{unknown}_data.csv")
...
class TestSubstituteModuleAttrs:
    ...
    def test_unknown_module_attr_raises(self):
        ctx = TemplateContext(module_attrs={"id": "kmeans"})
        with pytest.raises(ValueError, match="module.unknown"):
            ctx.substitute("{module.unknown}.csv")
...
class TestSubstituteParams:
    ...
    def test_unknown_param_raises(self):
        ctx = TemplateContext()
        params = Params({"k": 3})
        with pytest.raises(ValueError, match="params.unknown"):
            ctx.substitute("{params.unknown}.csv", params=params)

    def test_no_params_object_raises(self):
        ctx = TemplateContext()
        with pytest.raises(ValueError, match="params.k"):
            ctx.substitute("{params.k}.csv")

    def test_none_params_raises(self):
        ctx = TemplateContext()
        with pytest.raises(ValueError, match="params.k"):
            ctx.substitute("{params.k}.csv", params=None)
...
class TestGatherContext:
    ...
    def test_gather_empty_provides_raises_on_unknown(self):
        ctx = TemplateContext(
            provides={},
        ...
        with pytest.raises(ValueError, match="dataset"):
            ctx.substitute("{dataset}_agg.csv")
