...
class TestModuleResolver:
    ...
    def test_resolve_without_commit_fails_in_production(self, temp_work_dir):
        """Test that resolve fails without commit (caught at model validation or resolver)."""
        from omnibenchmark.model import Module, Repository
        import pydantic

        # Empty commit is rejected at model construction time (Pydantic validation)
        with pytest.raises((pydantic.ValidationError, RuntimeError)):
            module = Module(
                id="test_module",
                repository=Repository(
                    url="https://github.com/test/repo.git", commit=""
                ),
