...
def mock_resolver():
    ...
    def mock_resolve(
        module, module_id, software_environment_id, dirty=False, unpinned=False
    ):
...
def collector_with_params():
    ...
    return MetricCollector(
        ...
        parameters=[Parameter(id="threshold", params={"threshold": [0.05, 0.01]})],
    )
...
class TestGatherCollectorInputs:
    ...
    def test_gather_from_single_stage(
        ...
        # Returns dict keyed by input_id; one input "metrics.scores" -> list of paths
        assert len(inputs) == 1
        paths = inputs["metrics.scores"]
        assert len(paths) == 2
        assert "metrics/method1/default/scores.json" in paths
        assert "metrics/method2/default/scores.json" in paths
    ...
    def test_gather_from_multiple_stages(self, mock_benchmark, sample_resolved_nodes):
        ...
        # Returns dict keyed by input_id: 2 keys, each with their paths
        assert len(inputs) == 2
        assert set(inputs.keys()) == {"metrics.scores", "results.data"}
        assert "metrics/method1/default/scores.json" in inputs["metrics.scores"]
...
class TestExpandCollectorParameters:
    ...
    def test_single_parameter(self):
        """Test expansion with single parameter (params format expands per value)."""
        collector = MetricCollector(
            id="test",
            name="Test",
            repository=Repository(url="http://test.git", commit="abc"),
            ...
            parameters=[Parameter(id="alpha", params={"alpha": [0.1, 0.5, 0.9]})],
        )

        result = _expand_collector_parameters(collector)

        # params format with a list value expands to 3 parameter combinations
        assert len(result) == 3
    ...
    def test_multiple_parameters(self):
        ...
        collector = MetricCollector(
            ...
            outputs=[IOFile(id="out", path="out.txt")],
            parameters=[
                Parameter(id="alpha", params={"alpha": [0.1, 0.5]}),
                Parameter(id="beta", params={"beta": [1, 2]}),
            ],
        )

        result = _expand_collector_parameters(collector)

        # Should expand to 2 + 2 = 4 combinations (one param per Parameter entry)
        assert len(result) == 4
        assert all(isinstance(p, Params) for p in result)
